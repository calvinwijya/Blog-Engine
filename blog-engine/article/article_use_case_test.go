...
func TestCreateArticleUseCase(t *testing.T) {
	...
	uc, err := NewArticleUseCase(nil)

	assert.Nil(t, uc)
	assert.Error(t, ErrNilStore, err)

	mem := CreateMemStore()
	uc1, err := NewArticleUseCase(mem)
	...
	for _, item := range tests {
		t.Run(item.Name, func(t *testing.T) {
			article, err := uc1.CreateArticle(ctx, item.Title, item.Content)
...
func TestDeleteArticle(t *testing.T) {
	...
	tests := []struct {
		...
		{"NonExistentID", nonExistentID, ErrArticleNotFound},
		{"ExistID", existID, nil},
	}
	...
	for _, item := range tests {
		t.Run(item.Name, func(t *testing.T) {
			err := uc.DeleteArticle(ctx, item.ID)
			assert.Equal(t, item.Err, err)
		})
	}
}
